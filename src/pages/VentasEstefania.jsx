...
const VentasEstefania = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        snapVentas.forEach(doc => {
          const d = doc.data();
          if (d.vendedor === 'estefania@digiwill.com') {
        ...
        snapClientes.forEach(doc => {
          if (doc.data().vendedor === 'estefania@digiwill.com') cCount++;
        });

        let pCount = 0;
        snapProductos.forEach(doc => {
          if (doc.data().vendedor === 'estefania@digiwill.com') pCount++;
        });
  ...
  return (
    <div className="p-8">
      <h2 className="text-3xl font-bold text-pink-400 mb-6 neon-text tracking-wider">Auditoría: Estefania</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="glass-panel p-6 rounded-xl flex items-center gap-4 border border-pink-500/30 shadow-[0_0_15px_rgba(236,72,153,0.15)]">
          <div className="p-4 bg-pink-500/20 rounded-lg text-pink-400">
            <ShoppingCart size={32} />
          </div>
          <div>
            <p className="text-sm text-slate-400 font-medium">Recaudo Total Estefania</p>
      ...
      <div className="glass-panel rounded-xl overflow-hidden border border-none">
        <div className="p-4 border-b border-white/5 bg-white/5">
          <h3 className="font-bold text-slate-200">Historial de Operaciones (Estefania)</h3>
        </div>
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {loading ? <tr><td colSpan="4" className="p-8 text-center text-slate-400">Cargando actividad de Estefania...</td></tr> : ventas.length === 0 ? (
              <tr><td colSpan="4" className="p-8 text-center text-slate-500">Estefania no ha registrado ventas aún.</td></tr>
            ) : (
              ventas.map(v => {
...
export default VentasEstefania;
